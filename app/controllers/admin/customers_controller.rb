  ...
  class CustomersController < BaseController
    ...
    def update
      if @customer.update(customer_params)
        redirect_to admin_customers_status_path(status: @customer.status),
                    notice: t('.success')
      else
        redirect_to edit_admin_product_path, alert: @customer.errors.full_messages
      end
    end

    def destroy
      @customer.destroy
      redirect_to admin_dashboard_path, notice: t('.success')
    end
